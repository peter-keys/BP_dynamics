...
final_frame = final_frame +3. 
	...
	; > This number is to make it easier to produce the info for a frame (so you don't have to change a lot of values in the file)
	; << Note that this is the number of the frame for a BP NOT the physical frame it is in (i.e., starts from 0 always) >>
fnum = 20

print,'Preparing frame ',fnum, ' of ', n_frames
print,' '
...
if(n_elements(d) eq 0) then begin &$
   ...
   d = float(dat[frame_num[fnum]]) &$ ;; for example snapshot 10 		; frame of MBP barycentre you're processing
   free_lun, lun &$	
endif
...
idl_to_nicole, file=folder+'/observed_f'+name+'.nic', i=obs[*,*,*,0], q = obs[*,*,*,1], u = obs[*,*,*,2], v = obs[*,*,*,3]

print,'Saving observed profiles to '+folder+'/observed_singlepix.nic'
...
idl_to_nicole, file='modelin_f'+name+'.nic', m = m1 ; do m=read_model('modelin.nic'), do help,m,/st  and plot: m.T[0,0,*],  m.el_p[0,0,*],  like this plot,m.tau[0,0,*],m.T[0,0,*] 
Print,'Producing input model called: modelin_f'+name+'.nic..'
