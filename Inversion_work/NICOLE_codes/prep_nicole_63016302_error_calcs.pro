...
;;
;;	Split the image up so you invert a 11 pacthes in 8 batches (to get 88 frames) of 150x150pix
;;		of a QS region to establish the errors in the inversions 
;;
...
; Check this value with the qs_checker_err.bat file to make sure it is a very quiet region
x0 = 186
x1 = 335
y0 = 258
y1 = 407
...
xsize = nx1
ny1 = y1 - y0 + 1
nx1 = nx1 * 11.    	; Split the data set up into your 
...
offset_arr1 = DBLARR(11)
offset_arr2 = DBLARR(11)
;FOR ff = 0, nf1-1 DO BEGIN &$
FOR ff = ((batch_num-1)*11), (batch_num*11)-1 DO BEGIN &$			; ONLY DO CERTAIN FRAMES... 
   print,'Processing frame '+STRCOMPRESS((STRING(FIX(ff))),/REMOVE_ALL)+' of '+STRCOMPRESS((STRING(FIX(11))),/REMOVE_ALL) &$
   openr, lun, file, /get_lun &$
   dat = assoc(lun, fltarr(nx, ny, nw, 4, /nozero), 512) &$
   d = FLOAT(dat[ff+3]) &$ 	;Correct for dodgy initial frames
   free_lun, lun &$	
   offsets = DBLARR(4) &$
   offsets[*] = 0. &$
   pos = [0,14,15,29] &$
   ...
   offset_arr1[ff-((batch_num-1)*11)] = offsetq1 &$
   dum2 = min(imeanq2, p) &$
   cc2 = parab_fit(wav2[p-1:p+1], imeanq2[p-1:p+1]) &$
   offsetq2 = cc2[1] / cc2[2] * 0.5d0 &$
   offset_arr2[ff-((batch_num-1)*11)] = offsetq2 &$
   ;; FIRST LINE
	   nw1 = N_ELEMENTS(iwav1) &$
	   cw1 = 6301.5d0 &$
	   red_satlas, min(wav1)+cw1 - 0.5, max(wav1) + cw1 + 0.2, x, y  &$ ;for 6301
   ...
   FOR yy = y0, y1 DO FOR xx = x0, x1 DO FOR ss = 0, 3 DO BEGIN &$
       obs[(xx-x0)+(xsize*(ff-((batch_num-1)*11))),(yy-y0),*,ss] = interpol(reform(d[xx,yy,*,ss]), float(iwav), float(nwav)) &$
   ENDFOR &$
ENDFOR
...
name = 'observed_error_'+STRCOMPRESS(STRING(FIX(batch_num)),/REMOVE_ALL)+'.nic'
idl_to_nicole, file=folder+'/'+name, i=obs[*,*,*,0], q = obs[*,*,*,1], u = obs[*,*,*,2], v = obs[*,*,*,3]
print,'Saving observed profiles to '+folder+'/'+name
SAVE,FILENAME=folder+'/error_'+STRCOMPRESS(STRING(FIX(batch_num)),/REMOVE_ALL)+'_xyf_info.sav',x0,x1,y0,y1,nx1,ny1,nf1,initial_frame,final_frame,xcoords,ycoords
...
idl_to_nicole, file='modelin_error_'+STRCOMPRESS(STRING(FIX(batch_num)),/REMOVE_ALL)+'.nic', m = m1 ; do m=read_model('modelin.nic'), do help,m,/st  and plot: m.T[0,0,*],  m.el_p[0,0,*],  like this plot,m.tau[0,0,*],m.T[0,0,*] 
