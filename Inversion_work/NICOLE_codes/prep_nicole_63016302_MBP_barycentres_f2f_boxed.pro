...
;;	Looking at including the barycentre pixel of each MBP in a frame.
;;	Allows me to invert all MBPs in a frame as one (for barycentre at least)
;;
...
;;
	; - Frame number of the frame to invert (as per tracking file...)

readcol,'/data/solarstore3/phk/data/27Jul2014/final_tracking_file',f='D,D,D,D,D,x', btpt_num, frame, xloc, yloc, area
;;
;; Looking to test the accuracy of the evolution by looking at a plot of a single MBP with a box about barycentre pixel 
;;

MBP = 279.	; <--- BP you want to look at.
fworking = 21	; <--- Frame that you are working on (to make it easier). Needs to start from zero		
...
fnum = frame_num[0] + fworking

print,'Preparing frame ',fnum
print,' '
...
if(n_elements(d) eq 0) then begin &$
   ...
   d = float(dat[fnum+3.]) &$ ;; for example snapshot 10 		; frame of MBP barycentre you're processing
   free_lun, lun &$	
endif
...
;.r verline.pro
print,'Plotting the theoretical line profiles with interpolated positions overplotted...'
print,'Check that positions are not too close to the telluric'
window,0,xsize=1200,ysize=800
plot,xfull,yfull,xtitle='Wavelength (A)',thick=2
FOR i = 0, N_ELEMENTS(nwav1)-1 DO verline,cw1+(nwav1[i]/1000.),line=2,thick=2,color=255
;oplot,cw1+(nwav[i]/1000.)
wait,5
FOR i = 0, N_ELEMENTS(nwav2)-1 DO verline,cw2+(nwav2[i]/1000.),line=2,thick=2 
wait,5
LOADCT,0,/SILENT
print,'Last on 1st line is:'
print,(MAX(nwav1)/1000.)+cw1 
print,' First on 2nd line is: ' 
print,(MIN(nwav2)/1000.)+cw2
...
; MBP 35 in the trimmed data set
;x0 = 807
;x1 = 846
;y0 = 721
;y1 = 759

; MBP 35 as 110x109 pixels (a lot of the early tests) in untrimmed data
;x0 = 772
;x1 = 881
;y0 = 686
;y1 = 794

;Single pixel Frame 5 MBP35 centre
;x0 = xcoords[fnum]
;x1 = xcoords[fnum]
;y0 = ycoords[fnum]
;y1 = ycoords[fnum]
...
; Make the box for the MBP here
x0 = xcoords[fworking] - 1
x1 = xcoords[fworking] + 1
y0 = ycoords[fworking] - 1
y1 = ycoords[fworking] + 1

nx1 = x1 - x0 + 1
ny1 = y1 - y0 + 1

;print,'Number of MBPs in this frame = ',N_ELEMENTS(xcoords)
;print,' '
...
plot,obs[0,0,*,0],title='Stokes I of single Pixel  that is to be Inverted (Intensity)'
wait,5
...
plot,obs[0,0,*,3],title='Stokes V of single Pixel that is to be Inverted (Stokes-V)'
wait,5
...
name = STRING(fworking,FORMAT='(I2.2)')
mbpname = STRING(MBP,FORMAT='(I2.2)')
idl_to_nicole, file=folder+'/observed_bary_MBP'+mbpname+'_f'+name+'.nic', i=obs[*,*,*,0], q = obs[*,*,*,1], u = obs[*,*,*,2], v = obs[*,*,*,3]

print,'Saving observed profiles to '+folder+'/observed_bary_fXX.nic'
...
idl_to_nicole, file='modelin_bary_MBP'+mbpname+'_f'+name+'.nic', m = m1 ; do m=read_model('modelin.nic'), do help,m,/st  and plot: m.T[0,0,*],  m.el_p[0,0,*],  like this plot,m.tau[0,0,*],m.T[0,0,*] 
Print,'Producing input model called: modelin_bary_f'+name+'.nic..'
...
;openw, 1, 'details_nicole_input_f'+name+'.dat', width = 300 &$
;Printf,1,' '
;Printf,1,'Deatails needed for NICOLE.input file...'
;printf,1,' '
;printf,1, 'Region 1'
;printf,1, 'Wavelength step = ', dlam1, ' mA'
;printf,1, 'First wavelength = ', nwav1[0]*1.d-3+6301.5012d0-offset1
;printf,1, 'number of wavelengths = ', np1
;printf,1, ' '
;printf,1, 'Region 2'
;printf,1, 'Wavelength step = ', dlam2, ' mA'
;printf,1, 'First wavelength = ', nwav2[0]*1.d-3+6302.4936d0-offset2
;printf,1, 'number of wavelengths = ', np2
;printf,1,' '
;close, 1
