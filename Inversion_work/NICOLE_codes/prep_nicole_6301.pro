...
print,'idx',idx
WRITEFITS,folder+'/idx_6301.fits',idx
WRITEFITS,folder+'/iwav_6301.fits',iwav
...
;;
wav = wav[0:15] ;For the single line calc, 6301, otherwiese [16:31]!
cw = 6301.5d0
...
for yy = y0, y1 do for xx = x0, x1 do for ss = 0, 3 do begin &$
   ;obs[xx-x0,yy-y0,*,ss] = interpol(reform(d[xx,yy,*,ss]), float(iwav), float(nwav))&$	; Original Change * in d[] to [0:15] for scan
    obs[xx-x0,yy-y0,*,ss] = interpol(reform(d[xx,yy,0:15,ss]), float(iwav), float(nwav)) &$
endfor
...
idl_to_nicole, file=folder+'/observed.nic', i=obs[*,*,*,0], q = obs[*,*,*,1], u = obs[*,*,*,2], v = obs[*,*,*,3]
...
; Going by what Aaron says, the first low nodes should have Q and U weights at very high values in the low nodes, then half of V in higher nodes
; >> REMEMBER TO CHANGE THE WEIGHTS TO WHAT YOU NEED
...
;   m1.gas_p[*,*,ii] = el[ii] &$
;endfor
;swrite_model2,'/data/solarstore3/phk/nicole_2018/NICOLE/run/modelin.mod',m1
...
;;
print, 'Wavelength Start =', nwav[0]*1.d-3 - offset + 6301.5012d0 ;; Assume that line center is at rest (can't do much better in the chromosphere). Use the lab wavelength in LINES
print, 'nwavelength=', np
print, 'dlambda=', string(dlam)+'mA'
end

;Y               FLOAT     = Array[115]
;IDL> normalized_intensity = interpol(y,x,(wav[nw-1] - offset)) * int_ratio
;% INTERPOL: V and X arrays must have same # of elements
;% Execution halted at: $MAIN$
;IDL> x = CONGRID(x, n)
;IDL> normalized_intensity = interpol(y,x,(wav[nw-1] - offset)) * int_ratio
;IDL> print,normalized_intensity
;  -0.00011525403
;IDL>
;IDL> scale = normalized_intensity / imean[nw-1]
;IDL> print,scale
;  -2.1750010e-07
;IDL> print,imean[nw-1]
;      529.903
;IDL> print,offset
;    -0.025598127
;IDL>
