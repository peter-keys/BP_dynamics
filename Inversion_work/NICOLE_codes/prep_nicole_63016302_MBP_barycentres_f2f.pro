...
fnum = 86		;<---- Change for the frame you want

readcol,'/data/solarstore3/phk/data/27Jul2014/final_tracking_file',f='D,D,D,D,D,x', btpt_num, frame, xloc, yloc, area
xcoords = xloc[WHERE (frame eq fnum)]  
ycoords = yloc[WHERE (frame eq fnum)]   				
;frame_num = frame[WHERE(frame eq fnum)]  
...
; Make an array of the barycentre pixels
dnew = FLTARR(N_ELEMENTS(xcoords),1,N_ELEMENTS(wav),4)
FOR x = 0, N_ELEMENTS(xcoords)-1 DO BEGIN &$
	dnew[x,0,*,*] = d[xcoords[x],ycoords[x],*,*] &$
ENDFOR

x0 = 0
x1 = N_ELEMENTS(xcoords)-1
y0 = 0
y1 = 0
...
for yy = y0, y1 do for xx = x0, x1 do for ss = 0, 3 do begin &$
    obs[xx-x0,yy-y0,*,ss] = interpol(reform(dnew[xx,yy,*,ss]), float(iwav), float(nwav)) &$
endfor
...
name = STRING(fnum,FORMAT='(I2.2)')
idl_to_nicole, file=folder+'/observed_bary_f'+name+'.nic', i=obs[*,*,*,0], q = obs[*,*,*,1], u = obs[*,*,*,2], v = obs[*,*,*,3]
...
idl_to_nicole, file='modelin_bary_f'+name+'.nic', m = m1 ; do m=read_model('modelin.nic'), do help,m,/st  and plot: m.T[0,0,*],  m.el_p[0,0,*],  like this plot,m.tau[0,0,*],m.T[0,0,*] 
