...
;;
;;	For a named MBP. This needs to be split in 2, so this is the 2nds batch of MBP frames.
;;
...
if(n_elements(d) eq 0) then begin &$
   ...
   d = float(dat[5]) &$ ;; for example snapshot 10 		; 30 is 174 USE 5 here as it is the 1st frame of this MBP (35) 
   free_lun, lun &$	
endif
...
WRITEFITS,folder+'/idx_6301_6302_MBPs.fits',idx
WRITEFITS,folder+'/iwav_6301_6302_MBPs.fits',iwav
...
;;
 ;;	Select an MBP to invert as one image (i.e., have sub-fields of MBP storing all frames along x) 
;;

; >> Want to put a few MBPs in the one frame to invert. 
; >> Each frame of the MBP must be in the image 
; >> Have each MBP as a band going left to right.
; >> Take a new y for a new MBP
; >> Take the widest point as the dimensions of the array
; >> There will be null points for MBPs that have different 
; >> lifetimes and distance crossed...
; >> Be sure to save out the info of x,y's and frames used 
; >> so that you can serparate the subsequent arrays out 
; >> afterwards....

;; 
  ;; INterpolated for several frames of an individual MBP
;;
readcol,'/data/solarstore3/phk/data/27Jul2014/final_tracking_file',f='D,D,D,D,D,x', btpt_num, frame, xloc, yloc, area

; >>>>>>>>>>>>>>>>>>> INSERT HERE THE MBP NUMBER OF THE ONE YOU WANT TO INVERT <<<<<<<<<<<<<<<<<<
MBP_num = 283.
im_dims = SIZE(d)
...
;nx1 = nx1 * nf1		; This value when using all frames in one.
mid = FIX(nf1/2.)
IF (mid * 2.) NE nf1 THEN (mid = mid + 1)

nx1 = nx1 * (nf1-mid)    ;(37-20+1)		;This value for cutting the thing in half... (2nd batch)

fs = INDGEN(nf1)+initial_frame

; >> NICOLE crashed out with a really long data set (i.e, 2750 x 109)
; >> Maybe try and change it so that the array is a square shape instead
; >> Might need ROUND(nf1/2.)*2 (EQ nf1, then it is even frames, NE nf1 it is uneven frames)
; >> Want to keep a whole frame on the same y plane

print,'Making an array with the dimensions: ['+STRCOMPRESS((STRING(FIX(nx1))),/REMOVE_ALL)+', '+STRCOMPRESS((STRING(FIX(ny1))),/REMOVE_ALL)+', '+STRCOMPRESS((STRING(FIX(np))),/REMOVE_ALL)+', 4]'

; QS x and y (for multiple frame stuff, i.e., you want to normlaise each frame individually to the same patch of QS)
xq0 = 140
xq1 = 390
yq0 = 100
yq1 = 390
...
;>> Offset_arrX stores the offsets for each frame for both lines. May need it later if there is significant shifts
offset_arr1 = DBLARR(nf1)
offset_arr2 = DBLARR(nf1)
;FOR ff = 0, nf1-1 DO BEGIN &$
FOR ff = mid, nf1-1 DO BEGIN &$			; ONLY DO CERTAIN FRAMES... 
   print,'Processing frame '+STRCOMPRESS((STRING(FIX(ff))),/REMOVE_ALL)+' of '+STRCOMPRESS((STRING(FIX(nf1))),/REMOVE_ALL) &$
   openr, lun, file, /get_lun &$
   dat = assoc(lun, fltarr(nx, ny, nw, 4, /nozero), 512) &$
   d = FLOAT(dat[fs[ff]]) &$ 	
   free_lun, lun &$	
   ...
   FOR ii = 1, 3 DO BEGIN &$
       ...
       print, 'Offsets Stk ',ii,' -> ', offsets[ii] &$
   ENDFOR &$
   imeanq1 = FLTARR(N_ELEMENTS(wav[0:15])) &$
   imeanq2 = FLTARR(N_ELEMENTS(wav[16:*])) &$
   imeanq1[*] = 0. &$
   imeanq2[*] = 0. &$
   FOR ww = 0, 15 do imeanq1[ww] = median(d[xq0:xq1, yq0:yq1, ww, 0]) &$
   FOR ww = 16, 31 do imeanq2[ww-16] = median(d[xq0:xq1, yq0:yq1, ww, 0]) &$
   dum1 = min(imeanq1, p) &$
   cc1 = parab_fit(wav1[p-1:p+1], imeanq1[p-1:p+1]) &$
   offsetq1 = cc1[1] / cc1[2] * 0.5d0 &$
   offset_arr1[ff] = offsetq1 &$
   dum2 = min(imeanq2, p) &$
   cc2 = parab_fit(wav2[p-1:p+1], imeanq2[p-1:p+1]) &$
   offsetq2 = cc2[1] / cc2[2] * 0.5d0 &$
   offset_arr2[ff] = offsetq2 &$
   ;; FIRST LINE
	   nw1 = N_ELEMENTS(iwav1) &$
	   cw1 = 6301.5d0 &$
	   red_satlas, min(wav1)+cw1 - 0.5, max(wav1) + cw1 + 0.2, x, y  &$ ;for 6301
	   ...
	   tr /= total(tr)  &$;; normalize by the area
	   y = fftconvol(y, tr) &$
	   normalized_intensityq1 = interpol(y,x,(wav1[nw1-1] - offsetq1)) * int_ratio &$
	   ;; Normalize observations
	   scaleq1 = normalized_intensityq1 / imeanq1[nw1-1] &$
   ;; SECOND LINE
	   nw2 = N_ELEMENTS(iwav2) &$
	   cw2 = 6302.5d0 ;for the prep 6302  &$
   	   red_satlas, min(wav)+cw2 - 0.2, max(wav) + cw2 + 0.5, x, y  &$;for 6302
	   ...
	   tr /= total(tr) &$;; normalize by the area
	   y = fftconvol(y, tr) &$
	   normalized_intensityq2 = interpol(y,x,(wav2[nw2-1] - offsetq2)) * int_ratio &$
	   scaleq2 = normalized_intensityq2 / imeanq2[nw2-1] &$
   ;; Normalize observations
   d[*,*,0:15,*] = d[*,*,0:15,*] * scaleq1 &$
   d[*,*,16:*,*] = d[*,*,16:*,*] * scaleq2 &$	
   FOR yy = y0, y1 DO FOR xx = x0, x1 DO FOR ss = 0, 3 DO BEGIN &$
       obs[(xx-x0)+(xsize*(ff-mid)),(yy-y0),*,ss] = interpol(reform(d[xx,yy,*,ss]), float(iwav), float(nwav)) &$
   ENDFOR &$
ENDFOR
...
;;
;;;with rebinning (don't use)
;;;obs = (rebin(obs[*,1:*,*,*], [nx1/2, ny1/2, np, 4]))[*,*,*,*]
name = 'observed_MBP'+STRCOMPRESS(STRING(FIX(MBP_num)),/REMOVE_ALL)+'_2.nic'
idl_to_nicole, file=folder+'/'+name, i=obs[*,*,*,0], q = obs[*,*,*,1], u = obs[*,*,*,2], v = obs[*,*,*,3]
print,'Saving observed profiles to '+folder+'/'+name
SAVE,FILENAME=folder+'/MBP'+STRCOMPRESS(STRING(FIX(MBP_num)),/REMOVE_ALL)+'_xyf_info.sav',x0,x1,y0,y1,nx1,ny1,nf1,initial_frame,final_frame,xcoords,ycoords

;------------------------------------------------------------------------------------------------------------------------------------------------------------------------
;				MULTI-MBP INVERSION: INTERPOLATE THE PROFILES
;------------------------------------------------------------------------------------------------------------------------------------------------------------------------
;	This method (commented out) takes several MBPs, makes sub-fields of them, and puts all the frames they
;	appear in, into one big array so they can be inverted all at the one time.
;	
;	That is fine, however, the array will be massive generally (~4500 x 450 for 4 reasonable MBPs) which would take 
;	~1.5 week(s) or more to process. If there is an issue that means you won't know for too long
;
;	I'll keep this here in case I need it later, but I will just focus on running MBPs individually at the moment (see above)
;
;	NB the second FOR LOOP (interpolating the array), has not been tested, so may fail....
;
; >> Add the MBP numbers you want from the tracking file
;MBP_nums = [21, 145, 35, 174]	; From Becky's thesis : 1st 2 are weak, 2nd 2 are strong MBPs
; >> This number of MBPs is perhaps too big... will take about a week to process
; >> Maybe only do one at a time/
; >> Start a loop to get the x, y's of MBPs and frames.
; >> Remember these need to be corrected to account for image/frame trimming done previously
;
;readcol,'/data/solarstore3/phk/data/27Jul2014/final_tracking_file',f='D,D,D,D,D,x', btpt_num, frame, xloc, yloc, area
;
;im_dims = SIZE(d)
;x_vals = DBLARR(N_ELEMENTS(MBP_nums),2)
;y_vals = DBLARR(N_ELEMENTS(MBP_nums),2)
;f_vals = DBLARR(N_ELEMENTS(MBP_nums),2)
;xdims = DBLARR(N_ELEMENTS(MBP_nums))
;ydims = DBLARR(N_ELEMENTS(MBP_nums))
;fdims = DBLARR(N_ELEMENTS(MBP_nums))
;FOR i = 0, N_ELEMENTS(MBP_nums)-1 DO BEGIN &$
;	MBP = MBP_nums[i] &$
;	xcoords = xloc[WHERE (btpt_num eq MBP_nums[i])] &$ 
;	ycoords = yloc[WHERE (btpt_num eq MBP_nums[i])] &$  				
;	frame_num = frame[WHERE (btpt_num eq MBP_nums[i])]  &$
;	initial_frame = frame_num[0] &$
;	final_frame = frame_num[N_ELEMENTS(frame_num) - 1]   &$
;	n_frames = N_ELEMENTS(frame_num) &$
	; > Correct values for trimming	
...
;	IF (x_max GT im_dims[1]) THEN x_max = im_dims[1] - 1. &$
;	IF (y_min LT 0) THEN y_min = 0. &$
;	IF (y_max GT im_dims[2]) THEN y_max = im_dims[2] - 1. &$	
	; > Save the values into arrays
;	x_vals[i,*] = [x_min, x_max] &$ 		; x values for the MBP subfield 
;	y_vals[i,*] = [y_min, y_max] &$  		; y values for the MBP subfield 
;	f_vals[i,*] = [initial_frame, final_frame] &$   ; frame values for the MBP lifetime
;	xdims[i] = x_max - x_min +1 &$			; sub field x dimensions
;	ydims[i] = y_max - y_min +1 &$			; sub field y dimensions
;	fdims[i] = final_frame - initial_frame +1 &$	; sub field frame numbers
;ENDFOR
; >> Calculate the biggest x and y dimensions
;xsizes = DBLARR(N_ELEMENTS(MBP_nums))
;FOR i = 0, N_ELEMENTS(MBP_nums)-1 DO xsizes[i] = xdims[i] * fdims[i] &$
;xfov = MAX(xsizes)
;yfov = TOTAL(ydims)	
;print,' Your array to be inverted will be: ['+arr2str(xfov,/trim)+', '+arr2str(yfov,/trim)+', '+arr2str(np,/trim)+', 4]'	
;; Interpolate new line positions. 
;obs  = fltarr(xfov, yfov, np, 4)
;obs[*] = 0.
;FOR nm = 0, N_ELEMENTS(MBP_nums)-1 DO BEGIN &$
;    FOR ff = 0, fdims[nm]-1 DO BEGIN &$ 
;	openr, lun, file, /get_lun &$
;   	dat = assoc(lun, fltarr(nx, ny, nw, 4, /nozero), 512) &$
;	d = FLOAT(dat[ff + f_vals[nm,0]) &$ 	
;   	free_lun, lun &$	
;	offsets = DBLARR(4) &$
;   	pos = [0,14,15,29] &$
;   	FOR ii = 1, 3 DO BEGIN &$
...
;	d[*,*,0:15,*] = d[*,*,0:15,*] * scale1 &$
;	d[*,*,16:*,*] = d[*,*,16:*,*] * scale2 &$
;	sub_d = d[x_vals[nm,0]:x_vals[nm,1], y_vals[nm,0]:y_vals[nm,1],*,*] &$
;	
;	FOR yy = y_vals[nm,0], y_vals[nm,1] -1 DO FOR xx = x_vals[nm,0], x_vals[nm,1] -1 DO FOR ss = 0, 3 DO BEGIN &$
;		IF (nm EQ 0) THEN obs[xx-x_vals[nm,0],(yy-y_vals[nm,0]),*,ss] = interpol(reform(sub_d[xx,yy,*,ss]), float(iwav), float(nwav)) &$
;		IF (nm GT 0) THEN obs[xx-x_vals[nm,0],(yy-y_vals[nm,0])+(TOTAL(ydims[0:nm-1])),*,ss] = interpol(reform(sub_d[xx,yy,*,ss]), float(iwav), float(nwav)) &$
;	ENDFOR &$
;    ENDFOR &$
;ENDFOR
;for yy = y0, y1 do for xx = x0, x1 do for ss = 0, 3 do begin &$
...
;------------------------------------------------------------------------------------------------------------------------------------------------------------------------
;			SINGLE FRAME INVERSION: SELECT A SUB-FIELD TO INVERT FOR A SINGLE FRAME (TESTING USUALLY....)
;------------------------------------------------------------------------------------------------------------------------------------------------------------------------
;;
 ;; Select FOV to invert (manual selection - previous version of selecting a region to invert)
;;
...
;;;with rebinning
;;;obs = (rebin(obs[*,1:*,*,*], [nx1/2, ny1/2, np, 4]))[*,*,*,*]
;idl_to_nicole, file=folder+'/observed_both.nic', i=obs[*,*,*,0], q = obs[*,*,*,1], u = obs[*,*,*,2], v = obs[*,*,*,3]
;print,'Saving observed profiles to '+folder+'/observed_both.nic'
;------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
printf, 1, weights &$
close, 1
WRITEFITS,folder+'/nwav_6301_6302_MBPs.fits',nwav
...
print, 'First wavelength = ', nwav1[0]*1.d-3+6301.5012d0-(MEDIAN(offset_arr1))
print, 'number of wavelengths = ', np1
print, ' '
print, 'Region 2'
print, 'Wavelength step = ', dlam2, ' mA'
print, 'First wavelength = ', nwav2[0]*1.d-3+6302.4936d0-(MEDIAN(offset_arr2))
