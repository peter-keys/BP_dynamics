FUNCTION total_wavelet_power_slice, datacube, cadence, first_period_in_sec, last_period_in_sec, sub_oct

;AUGMENTED TO FIND THE POWER OF JUST A SLICE WITH THIS PARTICULAR METHOD

lightcurve = REFORM(datacube[0,*])
...
xsize = N_ELEMENTS(datacube[*,0])
zsize = N_ELEMENTS(datacube[0,*])
...
new_datacube = FLTARR(xsize,zsize,num_periods)

print,''
print,'Power cube will be : '+ARR2STR(xsize,/trim)+'x by '+ARR2STR(zsize,/trim)+'y by '+ARR2STR(num_periods,/trim)+' periods'
print,''

image_ave = TOTAL(datacube,2) / zsize
LOADCT,0,/silent
...
FOR a = 0,(xsize-1) DO BEGIN
	...
	systm = SYSTIME(1)
    	loop_time = (systm - t1)
    	t1 = systm
	minutes = (FIX(((loop_time)*((xsize*zsize) - countup))/60.))
	...
	IF b MOD 100 eq 0 THEN print,'I am '+ARR2STR(((countup/(xsize*zsize))*100.),/trim) + $
